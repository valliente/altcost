...
export const RevenuePotentialCard: React.FC<RevenuePotentialCardProps> = memo(({ value = 2500.09 }) => {
  ...
  return (
    <div className="light-card p-6 flex flex-col justify-between h-full min-h-[220px]">
      ...
      <div className="my-2 space-y-1">
        <span className="text-xs text-slate-400 font-medium">Revenue Potential</span>
        <h3 className="text-2xl font-bold text-slate-900 font-mono">
          ${value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
        </h3>
      </div>

      {/* Smooth Wavy Yellow Line Chart */}
      <div className="h-16 w-full -mb-2">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={lineData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
            <defs>
              <linearGradient id="yellowGrad" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#fbbd08" stopOpacity={0.4} />
                <stop offset="100%" stopColor="#fbbd08" stopOpacity={0.0} />
              </linearGradient>
            </defs>
            <Area
              type="monotone"
